...
- `MultiSelect` 的 `inline` 形态：标题行现在占满整行，`全选 / 清空` 靠右。页面如果为此在 `legend` 上写过宽度补丁，可以删掉。

## 1.2.7 浮层与趋势图修复

- 业务侧可删除 Drawer、Modal 的 DOM 观察桥接和无障碍包装，直接使用公共组件；嵌套浮层的 inert、Escape、Tab 和焦点归还由组件库负责。
- 业务侧可删除对 `TrendAreaChart` SVG 与数据点角色的运行时改写，公共组件已直接输出 group 与可读数据点语义。
